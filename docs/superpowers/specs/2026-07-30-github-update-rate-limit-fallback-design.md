...
本次修复发布为 `v1.1.1`，同步到 GitHub `main` 并创建新的最小 Windows
ZIP Release。不修改数据库、数据目录、ZIP 安装流程或启动时行为，也不增加
自动检查更新。
...
- 使用当前已耗尽配额的真实网络环境验证“检查更新”能够通过网页降级识别
  新发布的 `v1.1.1`。
- 运行完整测试、重新打包并同步本地稳定安装。
- 推送 GitHub `main`，创建 `v1.1.1` Release，并上传精确命名为
  `autumn-recruitment-ledger-Windows-x64.zip` 的最小安装包。
- 发布后核对 Release 非草稿、非预发布，附件 SHA-256 与本地文件一致，直接
  下载返回 HTTP 200。
