...
def test_startup_logs_when_existing_new_database_wins_without_private_data(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, qapp: object
) -> None:
    old_paths = AppPaths.from_root(tmp_path / "AutumnRecruitmentLedger")
    new_paths = AppPaths.from_root(tmp_path / "RecruitmentRecordLedger")
    private_legacy_value = "旧记录-联系人-13800000000"
    private_new_value = "新记录-完整岗位描述"
    create_legacy_database(old_paths.database_path, company=private_legacy_value)
    create_legacy_database(new_paths.database_path, company=private_new_value)
    startup_databases: list[Database] = []
    ...
    try:
        assert application.run() == 0
        for handler in application.logging.getLogger().handlers:
            handler.flush()
        log_text = new_paths.log_path.read_text(encoding="utf-8")
        assert "新数据库已存在" in log_text
        assert "跳过旧数据迁移" in log_text
        assert private_legacy_value not in log_text
        assert private_new_value not in log_text
    finally:
        for database in startup_databases:
            database.close()


def test_generic_migration_error_reports_exact_legacy_database_path(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, qapp: object
) -> None:
    old_paths = AppPaths.from_root(tmp_path / "AutumnRecruitmentLedger")
    new_paths = AppPaths.from_root(tmp_path / "RecruitmentRecordLedger")
    old_paths.database_path.parent.mkdir(parents=True)
    old_paths.database_path.write_bytes(b"preserved legacy data")
    messages: list[str] = []

    def fail_migration(new_paths: AppPaths, old_root: Path) -> bool:
        raise application.DataMigrationError("旧数据库未通过 SQLite 完整性检查。")
    ...
    monkeypatch.setattr(AppPaths, "from_standard_paths", lambda: new_paths)
    monkeypatch.setattr(application, "legacy_data_root", lambda root: old_paths.root)
    monkeypatch.setattr(application, "migrate_legacy_data", fail_migration)
    monkeypatch.setattr(application.QMessageBox, "critical", record_error)

    assert application.run() == 1
    assert messages and str(old_paths.database_path) in messages[0]
    assert str(old_paths.database_path).endswith("data\\autumn_recruitment.db")
    assert old_paths.database_path.read_bytes() == b"preserved legacy data"
